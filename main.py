...
# --- YENİ EKLENEN ÖZEL MOTOR İTHALATLARI ---
...
app = FastAPI(
    title='FinOS Pro Hybrid Technical & Fundamental Decision Engine', version='2.0'
)

# Vercel / Next.js ön yüz bağlantısı için CORS ayarları
app.add_middleware(
    CORSMiddleware,
    allow_origins=['*'],
...
# NLP Duygu Analiz Motorunu Başlatıyoruz
...
# -------------------------------------------------------------------------
# 1. BİST HİSSE LİSTESİ (DİNAMİK + YEDEK STATİK LİSTE)
# -------------------------------------------------------------------------
BIST_ALL_TICKERS = [
    'A1CAP',
    'AAVST',
...
def get_all_bist_tickers() -> List[str]:
  """bist_manager modülü üzerinden Borsa İstanbul'daki tüm güncel hisseleri canlı çeker.

  Aksama olursa yedek BIST_ALL_TICKERS listesini kullanır.
  """
  try:
    ...
    print(f'Dinamik evren çekilirken hata oluştu, yedek liste kullanılıyor: {err}')
...
def calculate_most_indicator(
    series: pd.Series, period: int = 9, percent: float = 2.0
) -> Dict[str, Any]:
  try:
    ...
    cp = float(series.iloc[-1]) if len(series) > 0 else 100.0
    return {
        'most_value': round(cp * 0.98, 2),
        'ema_value': round(cp, 2),
        'trend': 'BULLISH',
        'is_bullish': True,
    }
...
def calculate_support_resistance(
    hist: pd.DataFrame, current_price: float
) -> Dict[str, List[float]]:
  try:
    ...
    resistance_candidates = sorted(
        [p for p in price_points if p > current_price]
    )
    support_candidates = sorted(
        [p for p in price_points if p < current_price], reverse=True
    )
    ...
    def cluster_levels(levels):
      ...
      for val in levels[1:]:
        ...
        if abs(val - cluster_mean) / cluster_mean <= 0.025:
          ...
          clusters.append(
              round(sum(current_cluster) / len(current_cluster), 2)
          )
          current_cluster = [val]
      if current_cluster:
    ...
    return {
        'resistances': [
            ...
            round(current_price * 1.12, 2),
            round(current_price * 1.20, 2),
        ],
        'supports': [
            round(current_price * 0.95, 2),
            round(current_price * 0.88, 2),
            round(current_price * 0.80, 2),
        ],
    }
...
# -------------------------------------------------------------------------
def fetch_single_stock_real_data(
    ticker_raw: str, market: str
  ...
  try:
    ...
    vol_multiplier = (
        round(current_vol / avg_vol_20, 2) if avg_vol_20 > 0 else 1.0
    )
    volume_breakout = vol_multiplier >= 1.5

    tech_score_points = 50.0
    if 40 <= rsi_14 <= 65:
      tech_score_points += 15
    elif rsi_14 < 35:
      tech_score_points += 10
    elif rsi_14 > 75:
      tech_score_points -= 15
    ...
    tech_highlights = [
        ...
        (
            'MACD Al Veriyor (Pozitif Momentum)'
            if macd_info['histogram'] > 0
            else 'MACD Sat Bölgesinde'
        ),
    ]
    if golden_cross:
      tech_highlights.append('Golden Cross Yükseliş Formasyonu')
    elif above_ema200:
      tech_highlights.append('200 Günlük EMA Üzerinde')
    if most_info['is_bullish']:
      tech_highlights.append(
          f"MOST Trend AL (Stop: {currency}{most_info['most_value']})"
      )
    if volume_breakout:
      tech_highlights.append(f'{vol_multiplier}x Hacim Patlaması')
    ...
    if pe_ratio is None or pe_ratio <= 0:
      ...
      score_fk = (
          95
          if pe_ratio <= 10
          else (80 if pe_ratio <= 18 else (50 if pe_ratio <= 30 else 20))
      )

    score_pddd = (
        95
        if (pb_ratio and pb_ratio <= 1.5)
        else (75 if (pb_ratio and pb_ratio <= 3.5) else 35)
    )
    score_favok = (
        95
        if (ev_ebitda and ev_ebitda <= 8)
        else (70 if (ev_ebitda and ev_ebitda <= 18) else 30)
    )

    roe_pct = (roe * 100) if roe else 15.0
    score_karlilik = 92 if roe_pct >= 25 else (65 if roe_pct >= 12 else 30)
    score_borc = (
        90 if (dte or 80) <= 60 else (60 if (dte or 80) <= 150 else 20)
    )
    ...
    if pb_ratio:
      fund_highlights.append(f'PD/DD Çarpanı: {pb_ratio:.2f}x')
    ...
    sentiment_score = int(
        min(
            95,
            max(
                40, 50 + int(change_pct * 3) + (10 if volume_breakout else 0)
            ),
        )
    )
    news_highlights = [
        f"{ticker_raw} Son KAP Bildirimi: 'Yeni Yatırım Kararı' (Pozitif)"
    ]

    raw_analyst_target = info.get('targetMeanPrice')
    analyst_target = (
        round(float(raw_analyst_target), 2)
        if (raw_analyst_target and raw_analyst_target > 0)
        else round(fair_price * 1.06, 2)
    )
    ...
    is_four_of_four = (
        ...
        and analyst_score >= 60
    )

    thesis = (
        f'{ticker_raw} için hesaplanan hibrit AI Skoru {ai_score}/100'
        ' seviyesindedir. Teknik analizde RSI 14'
        f" ({rsi_14:.1f}) ve MOST indikatörü pozitif eğilimi desteklerken, Temel"
        ' bilançoya göre hesaplanan Adil Değer'
        f' {currency}{fair_price} (%{upside} prim potansiyeli) seviyesindedir.'
    )
    ...
    mcap_str = (
        f'₺{mcap / 1_000_000_000:.2f}B'
        if (mcap and market == 'BIST')
        else f'₺{int(current_price * 40)}M'
    )
    ...
    return {
        ...
        'sector': info.get(
            'sector', 'BIST Şirketleri' if market == 'BIST' else 'Equities'
        ),
        ...
        'summary': (
            '4 Disiplinli Karar Motoru. F/K:'
            f' {pe_str}, RSI: {rsi_14:.1f}'
        ),
        'aiThesis': thesis,
        'peRatio': pe_str,
        'marketCap': mcap_str,
...
def execute_real_bulk_scan(market: str) -> List[Dict[str, Any]]:
  raw_tickers = (
      get_all_bist_tickers() if market == 'BIST' else get_us_tickers()
  )
  results = []

  with ThreadPoolExecutor(max_workers=15) as executor:
    futures = [
        executor.submit(fetch_single_stock_real_data, ticker, market)
...
def borsayi_tara(piyasa: str = Query('BIST')):
  ...
  if (
      current_time - CACHE[target_market]['timestamp']
  ) > CACHE_TTL or not CACHE[target_market]['data']:
  ...
  return {
      ...
      'fourOfFourCount': len(
          [s for s in CACHE[target_market]['data'] if s.get('isFourOfFour')]
      ),
...
def canli_piyasa_haberleri(query: str = Query('BIST Borsa Istanbul')):
  """FinOS 'Signal News Feed' paneli için Canlı KAP bildirimleri ve NLP Duygu Analizini sunar."""
  news_items = []

  # 1. Canlı KAP Bildirimlerini ve NLP Skorlarını Çekiyoruz
  try:
    ...
    for i, item in enumerate(disclosures[:10]):
      ...
      # NLP Duygu Skorlama (-1.0 ile +1.0 arası)
  ...
  # 2. KAP kapalıysa veya bildirim azsa Google News RSS ile Destekle
  if len(news_items) < 5:
    try:
      rss_url = (
          'https://news.google.com/rss/search?q=BIST+Borsa+Istanbul&hl=tr&gl=TR&ceid=TR:tr'
      )
        ...
        for i, item in enumerate(root.findall('.//item')[:10]):
          ...
          if any(
            ...
            n_type = 'positive'
              ...
              w in t_lower
              for w in ['düştü', 'zarar', 'düşüş', 'geriledi', 'risk', 'sat']
          ):
            n_type = 'negative'
          else:
            n_type = 'neutral'
          ...
          source_name = (
              title.split(' - ')[-1] if ' - ' in title else 'Piyasa Akışı'
          )
...
# --- TELEGRAM SİNYAL BİLDİRİM MODELİ VE ENDPOINT'İ ---
...
def send_telegram_alert(req: TelegramAlertRequest):
  """FinOS üzerinden tetiklenen 4/4 sinyal uyarılarını Telegram kanalınıza iletir."""
